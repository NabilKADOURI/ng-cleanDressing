...
export class EntityService {
  ...
  getSolution(): Observable<ApiListResponse<SolutionInterface>> {
    return this.http.get<ApiListResponse<SolutionInterface>>(
      `${this.url}/services`
    );
  }

  getArticle(): Observable<ApiListResponse<ArticleInterface>> {
    return this.http.get<ApiListResponse<ArticleInterface>>(
      `${this.url}/articles`
    );
  }

  getCategory(): Observable<ApiListResponse<CategoryInterface>> {
    return this.http.get<ApiListResponse<CategoryInterface>>(
      `${this.url}/categories`
    );
  }

  getProduct(): Observable<ApiListResponse<ProductInterface>> {
    return this.http.get<ApiListResponse<ProductInterface>>(
      `${this.url}/products`
    );
  }
  ...
  getCategoriesWithProducts(): Observable<CategoryInterface[]> {
    return this.http.get<CategoryInterface[]>(
      `${this.url}/categories?include=products`
    );
  }

  getMatter(): Observable<ApiListResponse<MatterInterface>> {
    return this.http.get<ApiListResponse<MatterInterface>>(
      `${this.url}/matters`
    );
  }

  getTestimonial(): Observable<ApiListResponse<TestimonialInterface>> {
    return this.http.get<ApiListResponse<TestimonialInterface>>(
  ...
  uploadProfilePicture(userId: number, formData: FormData): Observable<any> {
    return this.http.post(
      `${this.url}/users/${userId}/upload-picture`,
      formData
    );
  }
}
